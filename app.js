...
const assetUrl=fileName=>new URL(fileName,document.baseURI).href;
const mountainPhotos=[
  assetUrl('site-image-02.jpg'),
  assetUrl('site-image-04.jpg'),
  assetUrl('site-image-05.jpg')
];
const gardenPhotos=[
  assetUrl('site-image-03.jpg')
];
...
function renderGallerySlide(){
  ...
  const requestedSrc=activeGalleryPhotos[activeGalleryIndex];
  img.hidden=false;
  img.alt=`Wedding gallery photo ${activeGalleryIndex+1} of ${activeGalleryPhotos.length}`;
  img.onerror=()=>{
    console.error('Gallery image failed to load:',requestedSrc);
    img.hidden=true;
    galleryModalImage.classList.add('image-load-error');
  };
  img.onload=()=>{
    img.hidden=false;
    galleryModalImage.classList.remove('image-load-error');
  };
...
function openGallery(card){
  ...
  if(selectedSet){
    ...
    activeGalleryPhotos.forEach((src,i)=>{
      ...
      image.alt='';
      image.loading='eager';
      image.onerror=()=>thumb.classList.add('image-load-error');
